...
class Achievement extends React.Component {
  ...
  handlerPaperZone = record => {
    router.push(
      `/paperZone?planId=${record.planId}&paperId=${record.paperId}&departId=${record.departId}&staffId=${record.staffId}`,
    );
  };
  ...
  pageChange = (page, pageSize) => {
    ...
    dispatch({
      type: 'achievement/getResultForStudent',
      payload: {
        page: {
          pageNum: page,
          pageSize: 10,
...

      {
        ...
        render: (text, record) => (
          <span>
            ...
            <Button type="link" onClick={e => this.handlerPaperZone(record, e)}>
              分布图
            </Button>
          </span>
        ),
...
export default connect(state => ({
  ...state.achievement,
  loading: state.loading.models.achievement,
}))(Achievement);
